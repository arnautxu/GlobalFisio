...
function MotionCarousel<T>(props: MotionCarouselProps<T>) {
  const {
    items,
    options,
    className,
  ...
  return (
    ...
    >
      <div className={cn('overflow-hidden', viewportClassName)} ref={emblaRef}>
        <div className={cn('flex touch-pan-y touch-pinch-zoom', trackClassName)}>
          {items.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className={cn(
                  'h-[var(--slide-height)] mr-[var(--slide-spacing)] basis-[var(--slide-size)] flex-none flex min-w-0',
                  slideClassName,
                )}
              >
                <motion.div
      ...
      <div className={cn('flex justify-between', controlsClassName)}>
        <Button size="icon" onClick={onPrev} disabled={prevDisabled}>
          <ChevronLeft className="size-5" />
        </Button>

        <div className={cn('flex flex-wrap justify-end items-center gap-2', dotsClassName)}>
          {scrollSnaps.map((_, index) => (
            <DotButton
              key={index}
              label={getItemLabel?.(items[index], index) ?? `Slide ${index + 1}`}
